...
from training.train_config_helpers import load_models_from_config
...
def main(model_name, expt_set, chrom, checkpoint_code, outfmt='npz', dataset='val'):
  ...
  config_file = config_dir + '{}/{}.json'.format(expt_set, model_name)
  with open(config_file, 'r') as jf:
    config = json.load(jf)

  train_dataset = config['data_kwargs']['dataset']
  ...
  imp_dir = output_dir+'{}_imputations/{}/{}/'.format(dataset, expt_set, model_name)
